...
class Loss_MRAE(nn.Module):
    ...
    def forward(self, outputs, label):
        ...
        mrae = torch.mean(error.reshape(-1)) #mrae = torch.mean(error.view(-1))
        return mrae
...
class Loss_RMSE(nn.Module):
    ...
    def forward(self, outputs, label):
        ...
        rmse = torch.sqrt(torch.mean(sqrt_error.reshape(-1))) #rmse = torch.sqrt(torch.mean(sqrt_error.view(-1)))
        return rmse
...
# Dictionary associating methods and learning rates
learning_rates_dict = {
    'mirnet': 4e-4,
    'mst_plus_plus': 4e-4,
    'mst': 4e-4,
