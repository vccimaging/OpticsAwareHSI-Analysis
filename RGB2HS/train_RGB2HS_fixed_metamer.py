...
def validate_step(val_loader=None, model=None, criterion=None):
    """
    Validate model on both standard and metamer data in the validation set.
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            if args.dataset_name == 'ICVL':
                # For ICVL, we crop the centeral 512 x 512 region for validation, to be consistent with others
...
def train(train_loader=None, val_loader=None, model=None, args=None):
    """
    Train the model with fixed metamers.
    ...
    # header
    logger.info(f"Fixed metamer, {args.noise_type} noise {args.npe} for PSF {args.psf_name} with {args.rgb_format} for {args.method}")

    # wandb logging
    if args.wandb:
        import wandb
        prefix = f"Fixed_metamer"
    ...
    if args.wandb:
        wandb.log(
            {'validation_loss/loss': val_loss, 'val_mrae': mrae_loss, 'val_rmse': rmse_loss, 'val_psnr': psnr_loss, 'val_sam': sam_loss},step=0
        )
...
if __name__ == "__main__":
    """
    Train models with fixed metamers.
    """

    parser = argparse.ArgumentParser(
        description='RGB2HS model training with fixed metamers',
    ...
    parser.add_argument('--result_root', type=str, default='./RGB2HS/train_exp_fixed_metamer', help='root path to training results')
    ...
    # output path
    args.outf = f"{args.result_root}/Fixed_metamer_{args.loss_func}_{args.rgb_format}_{args.noise_type}{args.npe}_{args.psf_name}_{args.method}_{args.dataset_name}"
    ...
    # load training data (standard + metamer)
    ...
    train_data = dtools.TrainDataset(
        ...
        data_join="both", # must be "both" for fixed metamers
        rgb_format=args.rgb_format, 
        bgr2rgb=True,
        aug=True,
        config=config
    ...
    val_data = dtools.ValidDataset(
        ...
        metamer_data_root=args.metamer_data_root,
        data_join="both",  # must be "both" for fixed metamers
        bgr2rgb=True, rgb_format=args.rgb_format
    )
    ...
    print("Start training with fixed metamers")
